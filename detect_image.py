...
def save_keypoints_to_csv(image_path, humans, peaks, csv_file):
    ...
    with open(csv_file, mode='a', newline='') as file:
        ...
        if file.tell() == 0:
            header = ['Image Name'] + [f'{name}' for name in keypoint_names]
        ...
        for i in range(len(humans[0])):
            ...
            for j in range(human.shape[0]):
                ...
                if k >= 0:
                    ...
                    keypoints.append((j, 0, 0))  
            
            if sum(1 for kp in keypoints if kp[1] != 0 and kp[2] != 0) < 1:
                continue

            writer.writerow([image_name] + [f'{kp[1]},{kp[2]}' for kp in keypoints])
def execute(img):
    start = time.time()
    data = preprocess(img)
...
parser.add_argument('--image', type=str, default='/home/thangphung/trt_pose/tasks/human_pose/test', help='Directory of images to process')
...
csv_file_path = '/home/thangphung/trt_pose/tasks/human_pose/keypoints_results.csv'
...
for filename in os.listdir(args.image):
    if filename.endswith(('.png', '.jpg', '.jpeg')): 
        ...
        img = image.copy()
        pilimg = execute_2(img_path, img, orgimg, csv_file_path)

        output_dir = '/home/thangphung/trt_pose/tasks/human_pose/results'
...
#def send_file_to_server():
   ...
   # username = "thang"  
    #remote_path = "/C/D/do_an/"  
    #scp_command = f"scp {csv_file_path} {username}@{server_ip}:{remote_path}"
    
#    try:
 #       subprocess.run(scp_command, shell=True, check=True)
  #      print("File CSV done.")
   # except subprocess.CalledProcessError as e:
    #    print(f"Send errol: {e}")
