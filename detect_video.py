...
cap = cv2.VideoCapture('/home/thangphung/trt_pose/tasks/human_pose/test1ng.mp4')
...
while cap.isOpened() and count<500:
    ...
    img = image.copy()
    pilimg = execute_2(img, orgimg, t)
    if not isinstance(pilimg, np.ndarray):
        pilimg = np.array(pilimg)


    if pilimg.shape[-1] == 3:  
        pilimg = cv2.cvtColor(pilimg, cv2.COLOR_RGB2BGR)


    out_video.write(pilimg) 
    count += 1
