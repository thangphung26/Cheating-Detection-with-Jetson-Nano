...
def execute_2(img, src):
    ...
    for i in range(counts[0]):
        ...
        if is_cheating:
            for idx, keypoints in enumerate(cheating_keypoints):
                
                xmin, ymin = float('inf'), float('inf')
                xmax, ymax = float('-inf'), float('-inf')
                #print(keypoints)
        
                x1 = round(keypoints[10][1] * WIDTH * X_compress)
                y1 = round(keypoints[10][0] * HEIGHT * Y_compress)
    ...
    print("FPS:%f "%(fps))
    draw.text((20, 20), f"FPS: {fps:.2f}", fill="green", font=ImageFont.load_default())    
